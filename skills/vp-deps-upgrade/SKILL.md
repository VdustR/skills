...
description: >-
  ...
  Boundary: for version bumps within the same library. Use vp-deps-migrate when
...
- **Context7**: Optional but recommended; install through the current agent's MCP or tool setup flow.
...
- **Boundary**: When a major upgrade (e.g., React 18→19) requires API migration, this skill handles it. Use `vp-deps-migrate` only when the primary intent is replacing one library with another.

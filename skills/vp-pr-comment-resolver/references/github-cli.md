...
- Top-level submitted reviews are a separate paginated collection on the pull
  request review surface. Their bodies can contain actionable feedback even
  when no review thread exists.
- GitHub exposes no reply mutation for a top-level submitted-review body. Reply
  in the PR conversation with the review author and canonical review link so the
  response remains attributable.
...
the issue-comment and submitted-review counts independently. For GraphQL,
paginate `reviews` and `reviewThreads` independently and, when a thread's
`comments.pageInfo.hasNextPage` is true, paginate that nested connection as
well. Preserve stable review, comment, and thread IDs and deduplicate by node
ID. Record each collection's count and final `hasNextPage: false` state.
...
Fetch enough pagination to prove no submitted review, review thread, inline
reply, or PR conversation issue comment was missed.
